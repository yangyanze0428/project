...
function dgcreator(pattern, url, query, scheme, options, formatter, funcolopt) {
    ...
    this.setdata = function (_query) {
        $(pattern).bootstrapTable('refresh', { url: urlgetlist, silent: true, query: _query });
    ...
    this.create = function () {
        ...
        $(pattern).bootstrapTable(_options);
        _isCreated = true;
    }

    this.setOption = function (key, value) {
        if (_isCreated) {
            $(pattern).bootstrapTable("options")[key] = value;
...
function openDlg(pattern, title, url, dlgSize, paras) {
    ...
    $("#" + pattern + " iframe").css("width", "100%").css("height", "100%").css("border", 0);
    $("#" + pattern).modal({
        title: title,
        minimizable: false,
        maximizable: false,
